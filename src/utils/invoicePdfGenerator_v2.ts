/**
 * INVOICE PDF GENERATOR - VERSION 2
 * ===============================
 * This file contains the enhanced invoice PDF generation functionality.
 * It has been updated from the original version to fix the logo stretching issue.
 * 
 * Features:
 ...
 * - Fixed company logo aspect ratio preservation
 * 
...
// Helper function to properly load an image and get its dimensions
  ...
  try {
    ...
    if (companyAssets.Logo?.dataUrl) {
      try {
        ...
        const maxWidth = 50; // Max width in mm for PDF
        const maxHeight = 30; // Max height in mm for PDF
        
        // Load the image and properly wait for it to load completely
        const img = await loadImage(companyAssets.Logo.dataUrl);
        
        // Calculate dimensions that preserve aspect ratio using the loaded image
        let logoWidth, logoHeight;
        const aspectRatio = img.width / img.height;
        
        if (aspectRatio > 1) {
          // Wider than tall - constrain by width
          logoWidth = Math.min(maxWidth, img.width / 5); // Reduce original width which is in pixels
          logoHeight = logoWidth / aspectRatio;
        } else {
          // Taller than wide or square - constrain by height
          logoHeight = Math.min(maxHeight, img.height / 5); // Reduce original height which is in pixels
          logoWidth = logoHeight * aspectRatio;
        }
        
        // Ensure reasonable minimum size
        logoWidth = Math.max(logoWidth, 20);
        logoHeight = Math.max(logoHeight, 10);
        
        // Also ensure we don't exceed maximums
        logoWidth = Math.min(logoWidth, maxWidth);
        logoHeight = Math.min(logoHeight, maxHeight);
        ...
        console.log('Logo added successfully with dimensions:', { logoWidth, logoHeight, aspectRatio });
      } catch (error) {
        console.error('Error adding logo:', error);
        yPos += 10; // Add some space even if logo fails
      }
    } else {
      yPos += 10; // Space where logo would have been
    }
    ...
    doc.setFontSize(12); // Slightly larger for company name heading
    doc.setFont('helvetica', 'bold');
    // Use actual company name from Company.tsx (stored in localStorage)
    doc.text(companyDetails.name || 'Company', leftColumnX, yPos);
    yPos += 5; // Slightly more space after company name heading
