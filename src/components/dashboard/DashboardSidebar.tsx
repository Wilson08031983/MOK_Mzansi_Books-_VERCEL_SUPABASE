...
const DashboardSidebar: React.FC<DashboardSidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 ${collapsed ? 'w-20' : 'w-72'} glass backdrop-blur-xl bg-white/10 dark:bg-black/30 border-r border-white/10 shadow-business-xl transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
        ...
        <ul className="space-y-2">
          {sidebarItems.map((item, index) => {
            ...
            const itemClass = `group flex items-center ${collapsed ? 'justify-center p-3' : 'px-4 py-4'} text-sm font-medium rounded-xl transition-all duration-300 ${
              ...
              'text-slate-700 dark:text-slate-200 hover:bg-white/10 hover:shadow-business hover-lift'
            }`;
            
            const iconClass = `h-5 w-5 ${collapsed ? '' : 'mr-4'} ${
              isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400 group-hover:text-mokm-purple-600'
            } transition-colors`;
            ...
            const contentElement = (
              ...
              >
                ...
                {!hasAccess && !collapsed && (
                  <LockIcon className="ml-auto h-4 w-4 text-slate-400 dark:text-slate-500" />
                )}
              </div>
            );
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent side="right" className="font-sf-pro flex items-center gap-2">
                        {t(`nav.${item.labelKey}`)}
                        {!hasAccess && <LockIcon className="h-3 w-3 text-slate-400 dark:text-slate-500" />}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                ) : (
      ...
      <div className="absolute bottom-6 left-2 right-2 space-y-2">
        {collapsed ? (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="w-full justify-center text-slate-700 dark:text-slate-300 hover:bg-white/10 hover:text-mokm-orange-600 transition-all duration-300" onClick={() => signOut()}>
          ...
          <Button variant="ghost" className="w-full justify-start text-slate-700 dark:text-slate-300 hover:bg-white/10 hover:text-mokm-orange-600 transition-all duration-300 font-sf-pro" onClick={() => signOut()}>
