    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h2 className="text-2xl font-bold text-slate-900 font-sf-pro">{t('quotations.createQuotation')}</h2>
          <Button
            variant="ghost"
            size="sm"
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          <div className="grid grid-cols-1 gap-6 mb-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-sf-pro">{t('quotations.basicInformation')}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="client" className="font-sf-pro">{t('quotations.client')}</Label>
                    <select
                      id="client"
                  ...
                  <div>
                    <Label htmlFor="reference" className="font-sf-pro">{t('quotations.reference')}</Label>
                    <Input
                      id="reference"
                      value={formData.reference}
                      onChange={(e) => setFormData(prev => ({ ...prev, reference: e.target.value }))}
                      placeholder={t('quotations.referencePlaceholder')}
                      className="font-sf-pro"
                    />
                  </div>
                  <div>
                    <Label htmlFor="date" className="font-sf-pro">{t('quotations.date')}</Label>
                    <Input
                      id="date"
                      type="date"
                      value={formData.date}
                  ...
                  <div>
                    <Label htmlFor="expiryDate" className="font-sf-pro">{t('quotations.expiryDate')}</Label>
                    <Input
                      id="expiryDate"
                      type="date"
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg font-sf-pro">{t('quotations.lineItems')}</CardTitle>
                  <Button
                    onClick={addItem}
                    size="sm"
                ...
                <table className="w-full min-w-[1200px] border-separate border-spacing-x-6 border-spacing-y-2">
                  <thead>
                    <tr className="font-semibold text-sm text-slate-600 font-sf-pro">
                      <th className="w-16 text-left pb-3">{t('quotations.itemNo')}</th>
                      <th className="w-[300px] text-left pb-3">{t('quotations.description')}</th>
                      <th className="w-24 text-center pb-3">{t('quotations.quantity')}</th>
                      <th className="w-32 text-center pb-3">{t('quotations.rate')}</th>
                      <th className="w-32 text-center pb-3">{t('quotations.markupPercent')}</th>
                      <th className="w-32 text-center pb-3">{t('quotations.discount')}</th>
                      <th className="w-36 text-right pb-3">{t('quotations.amount')}</th>
                      <th className="w-10 text-center pb-3"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {lineItems.map((item, index) => (
                      <tr key={item.id} className="align-middle font-sf-pro">
                        ...
                        <td className="py-3 align-middle">
                          <Input
                            ...
                            placeholder={t('quotations.enterDescription')}
                          />
                        </td>
                        <td className="py-3 align-middle">
                          <Input
                            ...
                            min="0"
                            className="w-full text-center font-sf-pro"
                          />
                        </td>
                        <td className="py-3 align-middle">
                          <Input
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg font-sf-pro">{t('quotations.termsAndNotes')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="terms" className="font-sf-pro">{t('quotations.termsAndConditions')}</Label>
                <textarea
                  id="terms"
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-mokm-purple-500 focus:border-mokm-purple-500 font-sf-pro"
                  placeholder={t('quotations.enterTerms')}
                />
              </div>
              <div>
                <Label htmlFor="notes" className="font-sf-pro">{t('quotations.notes')}</Label>
                <textarea
                  id="notes"
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-mokm-purple-500 focus:border-mokm-purple-500 font-sf-pro"
                  placeholder={t('quotations.enterNotes')}
                />
              </div>
            </CardContent>
          </Card>
            ...
            <Button
              ...
              onClick={() => {
                try {
                  ...
                  if (!client) {
                    toast.error(t('quotations.selectClientBeforePreviewing'));
                    return;
                  }
                  ...
                  toast.error(t('quotations.previewError'));
                }
              }}
            >
        ...
        <ErrorBoundary fallback={<div>{t('quotations.previewErrorFallback')}</div>}>
          {isPreviewModalOpen && (
