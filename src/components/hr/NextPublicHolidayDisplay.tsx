...
const NextPublicHolidayDisplay: React.FC<NextPublicHolidayDisplayProps> = ({ compact = false }) => {
  ...
  if (compact) {
    return (
      <div className="flex items-center justify-between bg-white/70 backdrop-blur-sm rounded-lg p-3 border border-white/20 shadow-sm animate-fade-in">
        ...
        <span className="text-xs bg-mokm-blue-100 text-mokm-blue-800 px-2 py-1 rounded-full">
  ...
  return (
    <Card className="glass backdrop-blur-sm bg-white/50 border border-white/20 shadow-business mb-4">
      <CardHeader className="pb-2">
        <CardTitle className="text-slate-900 font-sf-pro text-sm flex items-center">
          <Calendar className="h-4 w-4 mr-2 text-mokm-purple-500" />
          Next Public Holiday
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col space-y-1">
          <div className="flex justify-between items-center">
            <span className="font-medium text-mokm-blue-700">
          ...
          {holiday.description && (
            <p className="text-xs text-slate-600">{holiday.description}</p>
          )}
