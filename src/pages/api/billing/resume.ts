...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (existing) {
      // If subscription already canceled, cannot resume; return snapshot idempotently
      ...
      // If cancel_at_period_end is already false, idempotent no-op
      ...
      // Flip cancel_at_period_end back to false
    ...
    // No subscription exists - if user is on trial via user.trialEndsAt, create a trial subscription and ensure cancel_at_period_end is false
    const now = new Date();
    ...
    // No subscription and no active trial => nothing to resume; return a canceled-like snapshot
    return res.status(200).json(
      buildSnapshot({
        status: 'canceled',
        planCode: null,
    ...
    console.error('Error in POST /api/billing/resume:', error);
